...
class  Hikiotoshi {
   ...
   public function bankInOut($post) {
    ...
    public function hyoji_ichiran($yy,$mm){
     $sql = "select * from bankInOut where yyyy=:yyyy and mm=:mm";
   ...
   public function create_komoku($post) {
    ...
    //カレンダー
    public function getBody1($yy,$mm){
      ...
      for($i=1;$i<=$wd1;$i++){
       ...
       while(checkdate($m,$d,$y)){
          ...
          if($aaa == $today){
             $getBody.="<td id=$d width='50px' class='today'><a href='usedHikiotoshi_hyoji.php?yy=$y&mm=$m&dd=$d'>$d</a><br>{$gokei}円</td>";

          }else{
             $getBody.="<td id=$d width='50px'><a href='usedHikiotoshi_hyoji.php?yy=$y&mm=$m&dd=$d'>$d</a><br>{$gokei}円</td>";
